...
type User struct {
	...
	PhoneNumber      string `gorm:"size:65;not null"               json:"phonenumber"`
	DateofBirth      string `gorm:"size:256;not null"            json:"dateofbirth"`
	DateofJoining    string `gorm:"size:256;not null"          json:"dateofjoining"`
	Address          string `gorm:"size:256;not null"                json:"address"`
	EmployerLocation string `gorm:"size:100;not null"            json:"employerlocation"`
}
...
func (u *User) Validate(action string) error {
	...
	default: // this is for creating a user, where all fields are required
		...
		if u.DateofJoining == "" {
			return errors.New("DateofJoining is required")
		}
		if u.EmployerLocation == "" {
			return errors.New("employerLocation is required")
		}
